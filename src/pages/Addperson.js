...
import {func_convertFrom24To12Format, func_stringBase64File, func_getBase64FromUrl } from './../context/Functions_1';
...
function Addperson() {
  ...
  const handleAddPerson=e=>{
    try{
      ...
      reference.current.querySelector('#urlLink').value = '';
      reference.current.querySelector('#hddn_img').src = '';
      setStarttime('');
      setEndtime('');
      setBase64Img('');
      setScheduleArray([]);
  ...
  const handleChooseImage = async(e)=>{
    let urlLink = reference?.current?.querySelector('#urlLink').value;
    if(urlLink===''){
      e.currentTarget.querySelector('#handlePicture').click()
    }else{
      let resp1 = await func_getBase64FromUrl(urlLink);
      // 6 bits/char ... 8 bit/byte ... 1000 bytes/KB...133%
      let size = resp1?.length*6/8/1000*1.33; // size in KB
      (size<1000) //1000KB
          ? setBase64Img(resp1)
  ...
  const handlePicture=e=>{
    try{
      let temp = (data) =>{
        // 6 bits/char ... 8 bit/byte ... 1000 bytes/KB...133%
        let size = data?.length*6/8/1000*1.33; // size in KB
        console.log(size+' KB');
        (size<1000) //1000KB
          ? setBase64Img(data)
          : func_snackbar(reference,'Upload must be < 1 MB')
      };
      func_stringBase64File(e.currentTarget, temp);
    }catch(err){console.log('handlePicture:'+err)}
  };

  const handleHddnImg = e =>{
    try{
      let elem = reference?.current?.querySelector('#hddn_img');
      elem.src = e;
    }catch(err){console.log('handleHddnImg:'+err)}
  };
          ...
          <table width='100%'>
            <tbody>
              ...
              <tr style={{boxShadow:"1px 1px 4px 0px #8888", borderRadius:'5px', backgroundColor:'var(--sec-backgroundColor)'}}>
                ...
                <td>
                  <center>
                    <h3 style={{color:'var(--main-textColor)'}}>
              ...
              <tr style={{boxShadow:"1px 1px 4px 0px #8888", borderRadius:'5px', backgroundColor:'var(--thir-backgroundColor)'}}>
                ...
                <tr style={{boxShadow:"1px 1px 4px 0px #8888", borderRadius:'5px',backgroundColor:'var(--sec-backgroundColor)'}}>
                  <td colSpan={2}>
                    <div style={{display:'flex', justifyContent:'space-around', alignItems:"center", padding:'5px 5px'}}>
                      ...
                      <h3 data-time="hddn_start_time" onClick={e=>handleTime(e)} style={{boxShadow:"1px 1px 4px 0px #8888",borderRadius:'5px', padding:'5px', width:'50%', marginRight:'2px', backgroundColor:'var(--thir-backgroundColor)'}}>
                        <span style={{color:'var(--main-textColor)'}}>
                          start:&nbsp;
                        </span>
                        <span style={{filter:'drop-shadow(1px 1px 0px palegreen)'}}>
                          {starttime!==''?func_convertFrom24To12Format(starttime):''}
                        </span>
                      </h3>
                      
                      <h3 data-time="hddn_end_time" onClick={e=>handleTime(e)} style={{boxShadow:"1px 1px 4px 0px #8888",borderRadius:'5px', padding:'5px', width:'50%', marginLeft:'2px', backgroundColor:'var(--thir-backgroundColor)'}}>
                        <span style={{color:'var(--main-textColor)'}}>
                          end:&nbsp;
                        </span>
                        <span style={{filter:'drop-shadow(1px 1px 0px tomato)'}}>
                          {endtime!==''?func_convertFrom24To12Format(endtime):''}
                        </span>
                      </h3>
                ...
                <tr>
                  <td colSpan={2}>
                    <div style={{display:'flex', justifyContent:'center', marginTop:'15px', alignItems:'center'}}>
                      ...
                      <div onClick={e=>handleChooseImage(e)} onMouseOver={e=>e.currentTarget.style.backgroundColor='#c0d6df'} onMouseOut={e=>e.currentTarget.style.backgroundColor='#e0fbfc'}
                ...
                <tr>
                  <td colSpan={2} style={{backgroundColor:'var(--sec-backgroundColor)', borderRadius:'5px', boxShadow:'1px 1px 4px 0px #8888'}}>
                    <div style={{display:'flex', justifyContent:'center'}}>
                      <span>
                        <InputBar ids='urlLink' type='url' placeholder='URL Link' func_onChange={handleHddnImg}/>
                      </span>
                      <span style={{width:'50px',display:'flex', justifyContent:'center'}}>
                        <img id='hddn_img' alt='' src='' crossOrigin="anonymous" style={{display:'block', maxHeight:'40px', width:'auto', maxWidth:'40px', boxShadow:'1px 1px 4px 0px #8888', borderRadius:'5px'}}/>
                      </span>
                    </div>
                  </td>
                </tr>
